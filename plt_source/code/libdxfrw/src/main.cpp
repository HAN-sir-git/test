...
int main() {
    ...
    //First read a dwg or dxf file
    dx_iface *input = new dx_iface();
    QString  path= "D:\\11.dxf";
    auto badState = input->fileImport( path.toStdString(), &fData, true);
    if (!badState) {
        std::cout << "Error reading file " << path.toStdString() << std::endl;
        return false;
    }
    for ( auto it=fData.blocks.begin(); it!=fData.blocks.end(); ++it)
    {
        auto ents = (*it)->ent;
\
    }
